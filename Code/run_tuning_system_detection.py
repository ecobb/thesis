...
from Modules.tuning_probability import calc_intonation_system
...
def run_tuning_system_change_detection(tuning_system_dict, key, with_key=False, num_notes=3, freq_A4=440.0):
    """
    ...
    # print(frequencies)
    # print(len(frequencies_new))

    predictions = []
    for i in range(len(frequencies)):
        # print(frequencies[i])
        label = calc_intonation_system(frequencies_new[i:i + num_notes], key, with_key, freq_A4)
        predictions.append(label)

    return predictions


def run_tuning_system_detection2(tuning_system_dict, key, with_key=False, num_notes_filter=3, min_len_seq=2,
    ...
    """
    predictions = run_tuning_system_change_detection(tuning_system_dict, key, with_key, num_notes_filter, freq_A4)
...
if __name__ == "__main__":
    ...
    test_dict_pythag = OrderedDict([((0.015, 0.48999), 219.97818774800268), ((0.515, 0.97499), 247.47898266722595), ((1.01, 1.49999), 277.2148749101121), ((1.52, 1.9849899999999998), 293.7422224486255), ((2.01, 2.49499), 329.9668386432649), ((2.52, 2.98499), 370.7376477432682), ((3.015, 3.4899899999999997), 417.1163051045462), ((3.51, 3.99999), 440.3001390444166)])


    # sequences_test = run_tuning_system_detection2(test_dict_pythag, num_notes_filter=2, min_len_seq=2,
    #                              time_thresh=.55, freq_A4=440.0)
    # print(sequences_test)
    test_predictions = run_tuning_system_change_detection(test_dict_pythag, key='A', with_key=True, num_notes=3, freq_A4=440.0)
    print(test_predictions)
    pythag_answer = np.array([220., 247.5, 278.4375, 293.33333333,
                              330., 371.25, 417.65625, 440.])
    edo_ratios = np.array([1, 2 ** (2 / 12), 2 ** (4 / 12), 2 ** (5 / 12), 2 ** (7 / 12), 2 ** (9 / 12), 2 ** (11 / 12), 2])
    edo_answer = 220 * edo_ratios
    pythag_freq = list(test_dict_pythag.values())
    just_ratios = np.array([1, 9/8, 5/4, 4/3, 3/2, 5/3, 15/8, 2])
    just_answer = 220.0*just_ratios
    # print(pythag_answer - pythag_freq)
    # print(just_answer)
    # print(edo_answer)
    # print(pythag_answer)
    for i in range(len(pythag_freq)):
        print(f"{pythag_freq[i]}, {test_predictions[i]}, just: {just_answer[i]}, edo: {edo_answer[i]}, pythag: {pythag_answer[i]}")
